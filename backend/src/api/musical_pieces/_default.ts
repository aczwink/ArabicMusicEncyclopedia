...
class _api_
{
    ...
    {
        const mp = await this.musicalPiecesController.QueryMusicalPieces(request.data, request.data.offset, request.data.limit);
        const count = await this.musicalPiecesController.QueryMusicalPiecesCount(request.data);
